...
for j in range(0, len(txt_files), group_size):
    if j + group_size <= len(txt_files):
        ...
        for i in range(0,5,1):
            ...
            x_ticks = np.linspace(900+i*20, 920+i*20, 5)  # 设置刻度的起始值、结束值和刻度数量
            x_tick_labels = np.linspace(0, 2048, 5)  # 设置刻度对应的标签
            plt.xticks(x_tick_labels, x_ticks)
            plt.colorbar()  # 添加颜色条
            # plt.axis('off')
            file_name = f"gsm_image_{z}{i}.png"
            plt.savefig(f"H:/gsm/{file_name}", bbox_inches='tight', pad_inches=0)  # 保存图像
            plt.close()
